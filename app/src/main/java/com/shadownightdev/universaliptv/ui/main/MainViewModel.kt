...
import com.shadownightdev.universaliptv.data.repository.PlaylistRepository
        ...
        viewModelScope.launch {
            try {
                val cats = withContext(Dispatchers.IO) {
                    val channelsList = repository.getChannelsByPlaylist(playlistId)
                    val categoriesLive = repository.getCategoriesByPlaylist(playlistId)
                    // Use a different approach - get categories from channels directly
                    val allChannels = repository.run {
                        val db = com.shadownightdev.universaliptv.data.db.AppDatabase.getDatabase(getApplication())
                        db.channelDao().getChannelsByPlaylistSync(playlistId)
                    }
